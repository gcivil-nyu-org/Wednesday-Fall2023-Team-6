...
class ResetPasswordTests(TestCase):
    ...
    def test_01_existing_user(self):
        ...
        response = self.client.post("/user/passwordReset/", {"user_email":self.user_email})
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "user/resetPassword/password_reset_confirm.html")
        
        response = self.client.post(f'/user/passwordResetConfirm/{uid}/{token}', {"password": "testpassword", "confirm-password": "testpassword"})
        self.assertRedirects(
            response=response,
            expected_url="/user/login/",
            status_code=302,
        ...
        print("\Completed: test for checking password reset of existing user")
    ...
    def test_02_invalid_token(self):
        ...
        response = self.client.post(f'/user/passwordResetConfirm/{uid}/invalid_password', {"password": "testpassword", "confirm-password": "testpassword"})
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        ...
        print("\Completed: test for checking password reset of existing user with invalid token")
    
    def test_03_nonexisting_user(self):
        print("\nRunning: test for checking password reset of nonexisting user")
        
        response = self.client.post("/user/passwordReset/", {"user_email": "invalid_email"})
        self.assertRedirects(
            response=response,
        ...
        print("\Completed: test for checking password reset of nonexisting user")